...
function Router:dispatch(req)
	...
	if type(result) ~= "table" then
		return (self.on_error or default_error)(req, "handler must return a table, got " .. type(result))
	end

	if type(result.status) ~= "number" then
		return (self.on_error or default_error)(req, "handler response must have a numeric status")
	end
